...
async def _spare_create_server(
    cls: Any,
    protocol_factory: _ProtocolFactory,
    host: Any,
    port: Any,
...
class SpareSelectorEventLoop(asyncio.SelectorEventLoop):
    ...
    async def create_server(self, *args, **kwargs) -> PausableServer:  # type: ignore[no-untyped-def]
        return await _spare_create_server(super(), *args, **kwargs)


if sys.platform == "win32":

    class SpareProactor(IocpProactor):
        ...
        async def _spare_accept_loop(self, listener: socket.socket) -> Tuple[socket.socket, Tuple[object, ...]]:
            while True:
                ...
                try:
                    return await self._spare_accept(listener)
                except WindowsError as exc:  # pylint: disable=E0602
                    if exc.winerror not in (_winapi.ERROR_NETNAME_DELETED, _winapi.ERROR_OPERATION_ABORTED):
                        raise

        def _spare_accept(self, listener: socket.socket) -> asyncio.Future[Tuple[socket.socket, Tuple[object, ...]]]:
            ...
            async def accept_coro(self: SpareProactor, future: asyncio.Future[object], conn: socket.socket) -> None:
        ...
        def accept(self, listener: socket.socket) -> asyncio.Future[Tuple[socket.socket, Tuple[object, ...]]]:
            coro = self._spare_accept_loop(listener)
            return asyncio.ensure_future(coro)

    class SpareProactorEventLoop(ProactorEventLoop):
...
class SparePolicy(asyncio.DefaultEventLoopPolicy):
    def new_event_loop(self) -> asyncio.AbstractEventLoop:
        ...
        if sys.platform == "win32":
            if sys.version_info >= (3, 8):
                ...
                loop_factory = SpareProactorEventLoop
            else:
                # separate condition so coverage can report when this is no longer used
                loop_factory = SpareSelectorEventLoop
        else:
            loop_factory = SpareSelectorEventLoop
...
def set_spare_policy(connection_limit: int) -> None:
    ...
    asyncio.set_event_loop_policy(SparePolicy())
