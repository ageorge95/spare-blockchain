...
from spare.daemon.server import service_launch_lock_path
from spare.rpc.rpc_server import RpcApiProtocol, RpcServer, RpcServiceProtocol, start_rpc_server
from spare.server.spare_policy import set_spare_policy
...
from spare.types.peer_info import PeerInfo, UnresolvedPeerInfo
from spare.util.ints import uint16
from spare.util.lock import Lockfile, LockfileError
from spare.util.network import resolve
...
class Service(Generic[_T_RpcServiceProtocol]):
    def __init__(
        ...
        *,
        ...
        on_connect_callback: Optional[Callable[[WSSpareConnection], Awaitable[None]]] = None,
        ...
        self._log.info(f"spare-blockchain version: {spare_full_version_str()}")
        ...
        self._server = SpareServer.create(
            advertised_port,
            node,
            peer_api,
            node_type,
    ...
    async def setup_process_global_state(self) -> None:
        ...
        proctitle_name = f"spare_{self._service_name}"
...
def async_run(coro: Coroutine[object, object, T], connection_limit: Optional[int] = None) -> T:
    if connection_limit is not None:
        set_spare_policy(connection_limit)
