...
from spare.consensus.blockchain import AddBlockResult
...
from spare.protocols.wallet_protocol import CoinState, RespondBlockHeader, RespondToCoinUpdates
...
from spare.server.peer_store_resolver import PeerStoreResolver
...
from spare.util.chunks import chunks
...
from spare.util.db_wrapper import manage_connection
from spare.util.errors import KeychainIsEmpty, KeychainIsLocked, KeychainKeyNotFound, KeychainProxyConnectionFailure
...
from spare.util.profiler import mem_profile_task, profile_task
from spare.util.streamable import Streamable, streamable
from spare.wallet.transaction_record import TransactionRecord
from spare.wallet.util.new_peak_queue import NewPeakItem, NewPeakQueue, NewPeakQueueTypes
from spare.wallet.util.peer_request_cache import PeerRequestCache, can_use_peer_request_cache
from spare.wallet.util.wallet_sync_utils import (
...
from spare.wallet.wallet_weight_proof_handler import WalletWeightProofHandler, get_wp_fork_point
...
class WalletNode:
    ...
    _server: Optional[SpareServer] = None
    ...
    def get_cache_for_peer(self, peer: WSSpareConnection) -> PeerRequestCache:
    ...
    async def _process_new_subscriptions(self) -> None:
        while not self._shut_down:
            ...
            peer: Optional[WSSpareConnection] = None
    ...
    def initialize_wallet_peers(self) -> None:
        ...
        if self.wallet_peers is None and connect_to_unknown_peers and not testing:
            self.wallet_peers = WalletPeers(
                ...
                self.config.get("dns_servers", ["dns-introducer.spare.net"]),
    ...
    def on_disconnect(self, peer: WSSpareConnection) -> None:
    ...
    async def on_connect(self, peer: WSSpareConnection) -> None:
        if self._wallet_state_manager is None:
            return None
    ...
    async def is_peer_synced(self, peer: WSSpareConnection, height: uint32) -> Optional[uint64]:
        # Get last timestamp
    ...
    def is_trusted(self, peer: WSSpareConnection) -> bool:
    ...
    async def state_update_received(self, request: wallet_protocol.CoinStateUpdate, peer: WSSpareConnection) -> None:
    ...
    def get_full_node_peer(self) -> WSSpareConnection:
        """
        Get a full node, preferring synced & trusted > synced & untrusted > unsynced & trusted > unsynced & untrusted
        """
        full_nodes: List[WSSpareConnection] = self.get_full_node_peers_in_order()
        if len(full_nodes) == 0:
            raise ValueError("No peer connected")
        return full_nodes[0]

    def get_full_node_peers_in_order(self) -> List[WSSpareConnection]:
        """
        ...
        synced_and_trusted: List[WSSpareConnection] = []
        synced: List[WSSpareConnection] = []
        trusted: List[WSSpareConnection] = []
        neither: List[WSSpareConnection] = []
        all_nodes: List[WSSpareConnection] = self.server.get_connections(NodeType.FULL_NODE)
    ...
    async def new_peak_wallet(self, new_peak: wallet_protocol.NewPeakWallet, peer: WSSpareConnection) -> None:
        if self._wallet_state_manager is None:
    ...
    async def new_peak_from_trusted(
        self, new_peak_hb: HeaderBlock, latest_timestamp: uint64, peer: WSSpareConnection
    ) -> None:
    ...
    async def new_peak_from_untrusted(self, new_peak_hb: HeaderBlock, peer: WSSpareConnection) -> bool:
        far_behind: bool = (
    ...
    async def long_sync_from_untrusted(self, syncing: bool, new_peak_hb: HeaderBlock, peer: WSSpareConnection) -> None:
    ...
    async def sync_from_untrusted_close_to_peak(self, new_peak_hb: HeaderBlock, peer: WSSpareConnection) -> bool:
        async with self.wallet_state_manager.lock:
    ...
    async def wallet_short_sync_backtrack(self, header_block: HeaderBlock, peer: WSSpareConnection) -> int:
    ...
    async def fetch_and_update_weight_proof(self, peer: WSSpareConnection, peak: HeaderBlock) -> int:
    ...
    async def validate_block_inclusion(
        self, block: HeaderBlock, peer: WSSpareConnection, peer_request_cache: PeerRequestCache
    ) -> bool:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], peer: WSSpareConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
    ...
    async def fetch_children(
        self, coin_name: bytes32, peer: WSSpareConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
