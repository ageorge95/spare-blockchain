...
from spare.types.coin_spend import CoinSpend, compute_additions
...
from spare.wallet.nft_wallet import nft_puzzles
from spare.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTWalletInfo
from spare.wallet.nft_wallet.nft_puzzles import NFT_METADATA_UPDATER, create_ownership_layer_puzzle, get_metadata_and_phs
from spare.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from spare.wallet.outer_puzzles import AssetType, construct_puzzle, match_puzzle, solve_puzzle
from spare.wallet.payment import Payment
from spare.wallet.puzzle_drivers import PuzzleInfo, Solver
from spare.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_for_pk,
)
from spare.wallet.trading.offer import (
    OFFER_MOD,
    OFFER_MOD_HASH,
    OFFER_MOD_OLD,
    OFFER_MOD_OLD_HASH,
...
from spare.wallet.uncurried_puzzle import uncurry_puzzle
from spare.wallet.util.compute_memos import compute_memos
from spare.wallet.util.transaction_type import TransactionType
from spare.wallet.util.wallet_sync_utils import fetch_coin_spend
from spare.wallet.util.wallet_types import AmountWithPuzzlehash, WalletType
from spare.wallet.wallet import CHIP_0002_SIGN_MESSAGE_PREFIX, Wallet
from spare.wallet.wallet_coin_record import WalletCoinRecord
from spare.wallet.wallet_info import WalletInfo
from spare.wallet.wallet_nft_store import WalletNftStore
...
class NFTWallet:
    ...
    async def puzzle_solution_received(self, coin_spend: CoinSpend, peer: WSSpareConnection) -> None:
    ...
    async def generate_signed_transaction(
        ...
        unsigned_spend_bundle, spare_tx = await self.generate_unsigned_spendbundle(
            payments,
            fee,
            coins=coins,
            nft_coin=nft_coin,
        ...
        if spare_tx is not None and spare_tx.spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, spare_tx.spend_bundle])
            spare_tx = dataclasses.replace(spare_tx, spend_bundle=None)
        ...
        if spare_tx is not None:
            tx_list.append(spare_tx)
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            ...
            spare_tx = await self.standard_wallet.create_tandem_spare_tx(
        ...
        return nft_spend_bundle, spare_tx
        ...
        for asset, amount in offer_dict.items():
            if amount < 0:
                ...
                if len(offered_coins) == 0:
                    raise ValueError(f"Did not have asset ID {asset.hex() if asset is not None else 'SPARE'} to offer")
    ...
    async def mint_from_did(
        ...
        spare_coins: Optional[Set[Coin]] = None,
        spare_change_ph: Optional[bytes32] = None,
        ...
        - The launcher coins are then spent along with the created eve spend
          and an spare spend that funds the transactions and pays fees.
        ...
        :param spare_coins: [Optional] For use with bulk minting to provide the coin used for funding the minting spend.
        This coin can be one that will be created in the future
        :param spare_change_ph: [Optional] For use with bulk minting, so we can specify the puzzle hash that the change
        ...
        :param fee: A fee amount, taken out of the spare spend.
        """
        # get DID Wallet
        ...
        # Ensure we have an spare coin of high enough amount
        assert isinstance(fee, uint64)
        total_amount = len(metadata_list) + fee
        if spare_coins is None:
            spare_coins = await self.standard_wallet.select_coins(uint64(total_amount))
        assert len(spare_coins) > 0
        ...
        for mint_number in range(mint_number_start, mint_number_end):
            ...
            intermediate_launcher_puz = did_wallet_puzzles.INTERMEDIATE_LAUNCHER_MOD.curry(
                spare.wallet.singleton.SINGLETON_LAUNCHER_PUZZLE_HASH, mint_number, mint_total
            )
            ...
            launcher_coin = Coin(
                intermediate_launcher_coin.name(), spare.wallet.singleton.SINGLETON_LAUNCHER_PUZZLE_HASH, amount
            )
            ...
            launcher_cs = CoinSpend(
                launcher_coin, spare.wallet.singleton.SINGLETON_LAUNCHER_PUZZLE, genesis_launcher_solution
            )
        ...
        # Create the spare spend to fund the minting.
        spend_value = sum([coin.amount for coin in spare_coins])
        change: uint64 = uint64(spend_value - total_amount)
        spare_spends = []
        if spare_change_ph is None:
            spare_change_ph = await self.standard_wallet.get_new_puzzlehash()
        spare_primaries = [
            AmountWithPuzzlehash({"puzzlehash": spare_change_ph, "amount": change, "memos": [spare_change_ph]})
        ]

        first = True
        for spare_coin in spare_coins:
            puzzle: Program = await self.standard_wallet.puzzle_for_puzzle_hash(spare_coin.puzzle_hash)
            if first:
                message_list: List[bytes32] = [c.name() for c in spare_coins]
                message_list.append(
                    Coin(spare_coin.name(), spare_primaries[0]["puzzlehash"], spare_primaries[0]["amount"]).name()
                )
                message: bytes32 = std_hash(b"".join(message_list))

                if len(spare_coins) > 1:
                    spare_announcement: Optional[Set[bytes]] = {message}
                else:
                    spare_announcement = None

                solution: Program = self.standard_wallet.make_solution(
                    primaries=spare_primaries,
                    fee=fee,
                    coin_announcements=spare_announcement,
                    coin_announcements_to_assert={Announcement(did_coin.name(), message).name()},
                )
                primary_announcement_hash = Announcement(spare_coin.name(), message).name()
            ...
            spare_spends.append(CoinSpend(spare_coin, puzzle, solution))
        spare_spend = await self.standard_wallet.sign_transaction(spare_spends)
        ...
        did_full_puzzle: Program = spare.wallet.singleton.create_singleton_puzzle(
            innerpuz,
        ...
        total_spend = SpendBundle.aggregate([signed_spend_bundle, spare_spend, *eve_spends])
        return total_spend

    async def mint_from_spare(
        self,
        metadata_list: List[Dict[str, Any]],
        target_list: Optional[List[bytes32]] = [],
        mint_number_start: Optional[int] = 1,
        ...
        """
        Minting NFTs from a single SPARE spend using intermediate launcher puzzle
        :param metadata_list: A list of dicts containing the metadata for each NFT to be minted
        :param target_list: [Optional] a list of targets for transfering minted NFTs (aka airdrop)
        :param mint_number_start: [Optional] The starting point for mint number used in intermediate launcher
        puzzle. Default: 1
        ...
        funding_coin = spare_coins.copy().pop()
        ...
        # Empty set to load with the announcements we will assert from SPARE to
        # match the announcements from the intermediate launcher puzzle
        ...
        for mint_number in range(mint_number_start, mint_number_end):
            ...
            # create an ASSERT_COIN_ANNOUNCEMENT for the SPARE spend. The
            # intermediate launcher coin issues a CREATE_COIN_ANNOUNCEMENT of
            # the mint_number and mint_total for the launcher coin it creates
            intermediate_announcement_message = std_hash(int_to_bytes(mint_number) + int_to_bytes(mint_total))
            coin_announcements.add(std_hash(intermediate_launcher_coin.name() + intermediate_announcement_message))

            # Create the launcher coin, and add its id to a list to be asserted in the SPARE spend
            ...
            # Annnouncements for eve spend. These are asserted by the spare spend
            announcement_message = Program.to([eve_fullpuz.get_tree_hash(), amount, []]).get_tree_hash()
        ...
        for spare_coin in spare_coins:
            ...
            if first:
                ...
                solution: Program = self.standard_wallet.make_solution(
                    primaries=spare_primaries + primaries,
                    fee=fee,
                    coin_announcements=spare_announcement if len(spare_coins) > 1 else None,
...
if TYPE_CHECKING:
    from spare.wallet.wallet_protocol import WalletProtocol
