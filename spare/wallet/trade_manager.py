...
from spare.wallet.db_wallet.db_wallet_puzzles import ACS_MU_PH
...
from spare.wallet.trading.offer import OFFER_MOD_OLD_HASH, NotarizedPayment, Offer
...
class TradeManager:
    ...
    By default, standard SPARE is supported but to support other types of assets you must implement certain functions on
    ...
    Puzzle Drivers:
      - See spare/wallet/outer_puzzles.py for a full description of how to build these
    ...
    async def coins_of_interest_farmed(
        self, coin_state: CoinState, fork_height: Optional[uint32], peer: WSSpareConnection
    ) -> None:
        """
    ...
    async def cancel_pending_offer_safely(
        ...
        for coin in Offer.from_bytes(trade.offer).get_cancellation_coins():
            ...
            # This should probably not switch on whether or not we're spending a SPARE but it has to for now
    ...
    async def _create_offer_for_ids(
        ...
        try:
            ...
            for id, amount in offer_dict.items():
                ...
                # asset_id can either be none if asset is SPARE or
                ...
                if amount > 0:
                    ...
                    # For the SPARE wallet also include the fee amount to the coins we use to pay this offer
                ...
                if asset_id is not None and wallet is not None:  # if this asset is not SPARE
            ...
            # The access of the sorted keys here makes sure we create the SPARE transaction first to make sure we pay fee
            # with the SPARE side of the offer and don't create an extra fee transaction in other wallets.
            for id in sorted(coins_to_offer.keys()):
                ...
                # This should probably not switch on whether or not we're spending SPARE but it has to for now
                if wallet.type() == WalletType.STANDARD_WALLET:
                    tx = await wallet.generate_signed_transaction(
                        abs(offer_dict[id]),
    ...
    async def check_offer_validity(self, offer: Offer, peer: WSSpareConnection) -> bool:
