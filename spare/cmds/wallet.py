...
from spare.cmds.check_wallet_db import help_text as check_help_text
from spare.cmds.cmds_util import execute_with_wallet
from spare.cmds.coins import coins_cmd
from spare.cmds.plotnft import validate_fee
from spare.wallet.transaction_sorting import SortKey
...
@click.option(
    "-ma",
    "--min-coin-amount",
    help="Ignore coins worth less then this much SPARE or CAT units",
    type=str,
    required=False,
    default="0",
)
@click.option(
    "-l",
    "--max-coin-amount",
    help="Ignore coins worth more then this much SPARE or CAT units",
    type=str,
    required=False,
    default="0",
)
...
def address_sign_message(wallet_rpc_port: Optional[int], fingerprint: int, address: str, hex_message: str) -> None:
    extra_params: Dict[str, Any] = {"address": address, "message": hex_message, "type": AddressType.SPARE}
    import asyncio
...
@wallet_cmd.command("make_offer", short_help="Create an offer of SPARE/CATs/NFTs for SPARE/CATs/NFTs")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option(
    "-m", "--fee", help="A fee to add to the offer when it gets taken, in SPARE", default="0", show_default=True
)
@click.option(
    "-r",
    "--reuse",
...
@click.option(
    "-m", "--fee", help="The fee to use when pushing the completed offer, in SPARE", default="0", show_default=True
)
@click.option(
    "-r",
    "--reuse",
...
@click.option(
    "-m", "--fee", help="The fee to use when cancelling the offer securely, in SPARE", default="0", show_default=True
)
...
def check_wallet_cmd(ctx: click.Context, db_path: str, verbose: bool) -> None:
    """check, scan, diagnose, fsck Spare Wallet DBs"""
    import asyncio

    from spare.cmds.check_wallet_db import scan
