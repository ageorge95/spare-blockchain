...
from spare.cmds.cmds_util import transaction_status_msg, transaction_submitted_msg
from spare.cmds.peer_funcs import print_connections
...
from spare.util.bech32m import bech32_decode, decode_puzzle_hash, encode_puzzle_hash
from spare.util.config import load_config, selected_network_address_prefix
from spare.util.default_root import DEFAULT_ROOT_PATH
from spare.util.ints import uint16, uint32, uint64
from spare.wallet.nft_wallet.nft_info import NFTInfo
from spare.wallet.outer_puzzles import AssetType
from spare.wallet.puzzle_drivers import PuzzleInfo
...
from spare.wallet.util.address_type import AddressType, ensure_valid_address
...
def print_transaction(tx: TransactionRecord, verbose: bool, name, address_prefix: str, mojo_per_unit: int) -> None:
    if verbose:
        ...
        spare_amount = Decimal(int(tx.amount)) / mojo_per_unit
        ...
        print(f"Amount {description}: {spare_amount} {name}")
...
async def send(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    final_fee: uint64 = uint64(int(fee * units["spare"]))  # fees are always in SPARE mojos
...
async def make_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    if [] in [offers, requests]:
        ...
        for item in [*offers, *requests]:
                        ...
                        if item in requests:
                            ...
                            if info.supports_did:
                                ...
                                royalty_asset_dict[name] = (
                                    encode_puzzle_hash(info.royalty_puzzle_hash, AddressType.SPARE.hrp(config)),
            ...
            if fee > 0:
                print()
                print(f"Including Fees: {Decimal(fee) / units['spare']} SPARE, {fee} mojos")
            ...
            if royalty_asset_dict != {}:
                ...
                for nft_id, summaries in royalty_summary.items():
                    ...
                    for summary in summaries:
                        divisor = units["spare"] if summary["asset"] == "SPARE" else units["cat"]
                ...
                for asset, requested_amount in total_amounts_requested.items():
                    divisor = units["spare"] if asset == "SPARE" else units["cat"]
            ...
            if confirmation not in ["y", "yes"]:
                ...
                with open(pathlib.Path(filepath), "w") as file:
                    ...
                    if offer is not None:
                        ...
                        print(
                            f"Use spare wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view status"
                        )
                    else:
...
async def print_trade_record(record, wallet_client: WalletRpcClient, summaries: bool = False) -> None:
    ...
    if summaries:
        ...
        print(f"Included Fees: {fees / units['spare']} SPARE, {fees} mojos")
    print("---------------")
...
async def take_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    for royalty_asset_id in nft_coin_ids_supporting_royalties_from_offer(offer):
        if royalty_asset_id.hex() in offered:
            ...
            royalty_asset_dict[encode_puzzle_hash(royalty_asset_id, AddressType.NFT.hrp(config))] = (
                encode_puzzle_hash(address, AddressType.SPARE.hrp(config)),
                percentage,
            )
    ...
    if royalty_asset_dict != {}:
        ...
        for fungible_asset_id in fungible_assets_from_offer(offer):
            fungible_asset_id_str = fungible_asset_id.hex() if fungible_asset_id is not None else "spare"
            if fungible_asset_id_str in requested:
                nft_royalty_currency: str = "Unknown CAT"
                if fungible_asset_id is None:
                    nft_royalty_currency = "SPARE"
                else:
    ...
    print(f"Included Fees: {Decimal(offer.fees()) / units['spare']} SPARE, {offer.fees()} mojos")
...
async def mint_nft(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    royalty_address = (
        ...
        else ensure_valid_address(args["royalty_address"], allowed_types={AddressType.SPARE}, config=config)
    )
    target_address = (
        None
        if not args["target_address"]
        else ensure_valid_address(args["target_address"], allowed_types={AddressType.SPARE}, config=config)
    )
...
async def transfer_nft(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    try:
        ...
        nft_coin_id = args["nft_coin_id"]
        config = load_config(DEFAULT_ROOT_PATH, "config.yaml")
        target_address = ensure_valid_address(args["target_address"], allowed_types={AddressType.SPARE}, config=config)
        fee: int = int(Decimal(args["fee"]) * units["spare"])
...
async def send_notification(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    amount: uint64 = uint64(Decimal(args["amount"]) * units["spare"])
    message: bytes = bytes(args["message"], "utf8")
    fee: uint64 = uint64(Decimal(args["fee"]) * units["spare"])

    tx = await wallet_client.send_notification(address, message, amount, fee)

    print("Notification sent successfully.")
    print(f"To get status, use command: spare wallet get_transaction -f {fingerprint} -tx 0x{tx.name}")
...
async def sign_message(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    if args["type"] == AddressType.SPARE:
