...
from spare.data_layer.data_layer_wallet import DataLayerWallet
from spare.pools.pool_wallet import PoolWallet
from spare.pools.pool_wallet_info import FARMING_TO_POOL, PoolState, PoolWalletInfo, create_pool_state
...
from spare.rpc.rpc_server import Endpoint, EndpointResult, default_get_connections
...
from spare.types.blockchain_format.coin import Coin, coin_as_list
...
from spare.types.signing_mode import SigningMode
...
from spare.util.errors import KeychainIsLocked
...
from spare.wallet.cat_wallet.cat_constants import DEFAULT_CATS
from spare.wallet.cat_wallet.cat_wallet import CATWallet
...
from spare.wallet.did_wallet.did_wallet_puzzles import (
    DID_INNERPUZ_MOD,
    match_did_puzzle,
    metadata_to_program,
    program_to_metadata,
...
from spare.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTInfo
from spare.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs
from spare.wallet.nft_wallet.nft_wallet import NFTWallet
from spare.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from spare.wallet.notification_store import Notification
from spare.wallet.outer_puzzles import AssetType
from spare.wallet.puzzle_drivers import PuzzleInfo, Solver
from spare.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import puzzle_hash_for_synthetic_public_key
from spare.wallet.singleton import create_singleton_puzzle
from spare.wallet.trade_record import TradeRecord
from spare.wallet.trading.offer import Offer
from spare.wallet.transaction_record import TransactionRecord
from spare.wallet.uncurried_puzzle import uncurry_puzzle
from spare.wallet.util.address_type import AddressType, is_valid_address
from spare.wallet.util.compute_hints import compute_coin_hints
from spare.wallet.util.compute_memos import compute_memos
from spare.wallet.util.transaction_type import TransactionType
from spare.wallet.util.wallet_sync_utils import fetch_coin_spend_for_coin_state
...
class WalletRpcApi:
    ...
    async def get_latest_singleton_coin_spend(
        self, peer: WSSpareConnection, coin_id: bytes32, latest: bool = True
    ...
    async def verify_signature(self, request) -> EndpointResult:
        ...
        # signatures made by `spare keys sign`, which uses BLS_MESSAGE_AUGMENTATION_HEX_INPUT
    ...
    async def get_offer_summary(self, request) -> EndpointResult:
        ...
        from spare.util.bech32m import bech32_decode, convertbits
        from spare.wallet.util.puzzle_compression import decompress_object_with_puzzles
    ...
    async def cancel_offers(self, request: Dict) -> EndpointResult:
        ...
        if cancel_all:
            ...
            asset_id = request.get("asset_id", "spare")
        ...
        key = None
        if asset_id is not None and asset_id != "spare":
    ...
    async def did_get_info(self, request) -> EndpointResult:
        ...
        return {
            ...
            "p2_address": encode_puzzle_hash(p2_puzzle.get_tree_hash(), AddressType.SPARE.hrp(self.service.config)),
    ...
    async def nft_mint_bulk(self, request) -> EndpointResult:
        ...
        spare_coin_list = request.get("spare_coins", None)
        spare_coins = None
        if spare_coin_list:
            spare_coins = set([Coin.from_json_dict(spare_coin) for spare_coin in spare_coin_list])
        spare_change_target = request.get("spare_change_target", None)
        if spare_change_target is not None:
            if spare_change_target[:2] == "spare":
                spare_change_ph = decode_puzzle_hash(spare_change_target)
            else:
                spare_change_ph = bytes32(hexstr_to_bytes(spare_change_target))
        else:
            spare_change_ph = None
        ...
        if mint_from_did:
            sb = await nft_wallet.mint_from_did(
                ...
                spare_coins=spare_coins,
                spare_change_ph=spare_change_ph,
            ...
            sb = await nft_wallet.mint_from_spare(
                metadata_list,
                mint_number_start=mint_number_start,
                mint_total=mint_total,
                target_list=target_list,
