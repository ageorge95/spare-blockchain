...
from spare.data_layer.data_layer_errors import OfferIntegrityError
from spare.data_layer.data_layer_util import OfferStore, ProofOfInclusion, ProofOfInclusionLayer, StoreProofs, leaf_hash
...
from spare.types.blockchain_format.serialized_program import SerializedProgram
...
from spare.wallet.db_wallet.db_wallet_puzzles import (
    ACS_MU,
    ACS_MU_PH,
    GRAFTROOT_DL_OFFERS,
    SINGLETON_LAUNCHER,
...
from spare.wallet.puzzles.singleton_top_layer_v1_1 import SINGLETON_LAUNCHER_HASH
from spare.wallet.sign_coin_spends import sign_coin_spends
from spare.wallet.trading.offer import NotarizedPayment, Offer
from spare.wallet.transaction_record import TransactionRecord
from spare.wallet.util.compute_memos import compute_memos
from spare.wallet.util.merkle_utils import _simplify_merkle_proof
from spare.wallet.util.transaction_type import TransactionType
from spare.wallet.util.wallet_sync_utils import fetch_coin_spend, fetch_coin_spend_for_coin_state
...
class DataLayerWallet:
    ...
    async def get_launcher_coin_state(self, launcher_id: bytes32, peer: WSSpareConnection) -> CoinState:
    ...
    async def create_tandem_spare_tx(
        ...
        spare_tx = await self.standard_wallet.generate_signed_transaction(
            amount=uint64(0),
    ...
    async def delete_mirror(
        self, mirror_id: bytes32, peer: WSSpareConnection, fee: uint64 = uint64(0)
    ) -> List[TransactionRecord]:
        ...
        if excess_fee > 0:
            spare_tx: TransactionRecord = await self.wallet_state_manager.main_wallet.generate_signed_transaction(
                uint64(1),
                new_puzhash,
            ...
            txs = [
                dataclasses.replace(
                    txs[0], spend_bundle=SpendBundle.aggregate([txs[0].spend_bundle, spare_tx.spend_bundle])
                ),
                dataclasses.replace(spare_tx, spend_bundle=None),
            ]
